...
# Steps:
# Upload Custom Price Book and report any errors.
# Assign the CPB to a customer
# Assign the CPB to the 

#Functions

def create_custom_price_book(api_key):
...
def custom_price_book_customer_assignment(api_key, price_book_id):
    base_url = 'chapi.cloudhealthtech.com'
    query = f'/v1/price_book_assignments'
    headers = {'Content-type': 'application/json', 'Authorization': f'Bearer {api_key}'} 
    body = json.dumps({"price_book_id": price_book_id, "target_client_api_id": config.customer_id})
    connection = http.client.HTTPSConnection(base_url, context=ssl._create_unverified_context())
    connection.request('POST', query, body, headers)
    response = connection.getresponse()
    body = json.loads(response.read().decode())
...
def main():
    #Work
    price_book_id = create_custom_price_book(config.api_key)
    customer_price_book_assignment_id = custom_price_book_customer_assignment(config.api_key, price_book_id)
    print(custom_price_book_customer_account_assignment(config.api_key, customer_price_book_assignment_id))
